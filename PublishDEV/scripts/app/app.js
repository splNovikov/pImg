...
define('injector', [], function () {
  ...
  */
	...
	function _injectToButton(notInjectedButton, insertStyleName) {
		notInjectedButton.className += ' ' + insertStyleName;
	}
});
'use strict';
...
require(['ImgSettings', 'injector'], function (ImgSettings, injector) {
	...
	function polling(ms) {
		...
		polling.interval = setInterval(function () {
			...
			var sendButtons = injector.findSendButtons(settings.vkElements.sendButtonsSelectors);
			...
			var notInjectedButtons = injector.filterNotInjectedButtons(sendButtons, settings.injectedStyleName);
