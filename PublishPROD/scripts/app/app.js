...
define('pImgSettings', [],
	function () {
		return {setting1: 'sett'}
	}

);

require(['pImgSettings'], function(pImgSettings){
	debugger
});
/*
(function init() {
    ...
    function _generateSinglePImg() {
        window.pImgExt = { // todo: remove it from window. we don't need it in window at all. cos we work in encapsulated scope
    ...
    function _onCompletedListener() {

        // we have to check here is it IM or not
        if (pImgExt.storedPathName === '/im') {
            _startHtmlInjection();
        }

        window.setInterval(function () {
            if (window.location.pathname !== pImgExt.storedPathName) {
                pImgExt.storedPathName = window.location.pathname;
                if (pImgExt.storedPathName === '/im') {
                    _startHtmlInjection();
                }
            }
